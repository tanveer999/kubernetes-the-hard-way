...
scp -i vagrant/root-ssh-key/root_key \
  downloads-binaries/client/etcdctl \
  downloads-binaries/controller/etcd \
  units/etcd.service \
  root@server:~/
...
Each etcd member must have a unique name within a etcd cluster. Set the etcd name to match the hostname of the current compute instance:
